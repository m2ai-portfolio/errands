...
const promptLines = [
  '1. Use search_restaurants to find the restaurant the customer named, plus comparable options.',
  '2. Call ONE restaurant at a time with call_restaurant, starting with the one the customer asked for.',
  ...
  '6. If pay_deposit is refused or declined, do not retry; tell the customer the table needs a deposit and ask them to decide.',
]

export function dinnerErrand(deps: DinnerDeps): ErrandModule & {
  handlers: {
    search(input: z.infer<typeof SearchInput>): Promise<Json>
    call(input: z.infer<typeof CallInput>): Promise<Json>
    deposit(input: z.infer<typeof DepositInput>): Promise<Json>
  }
} {
  ...
  const handlers = {
    ...
    async call(input: z.infer<typeof CallInput>): Promise<Json> {
      ...
      const result = await deps.runCall({
        to,
        assistant: buildReservationAssistant(reservation, deps.voice),
      })
    ...
    [
      ...
      (input) => {
        ...
        return {
          ...
          category: 'restaurant_deposit',
          triedFirst: 'called the restaurant, which quoted this deposit',
        }
      },
    ],
  ])
  ...
  return { name: 'dinner', tools, spendTools, promptLines, handlers }
}
